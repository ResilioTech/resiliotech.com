/**
 * Resilio Tech Website JavaScript
 * Modern, performant code for interactive features
 */

class ResilioTechWebsite {
    constructor() {
    ...
    init() {
        ...
        this.setupHomeResourcePosts();
        this.setupHomeProjects();
    }
    ...
    async handleFormSubmission() {
        ...
        try {
            ...
            this.showFormMessage('Thank you! Your message has been sent successfully. We\'ll get back to you within 24 hours.', 'success');
            this.contactForm.reset();
            
        } catch (error) {
            console.error('Form submission error:', error);
            this.showFormMessage('Sorry, there was an error sending your message. Please try again or email us directly at contact@resiliotech.com.', 'error');
        } finally {
            // Reset button state
    ...
    /**
     * Load featured resource posts on home page
     */
    setupHomeResourcePosts() {
        const homeResourcesGrid = document.getElementById('home-resources-grid');
        
        if (homeResourcesGrid && typeof resourcesData !== 'undefined') {
            // Get exactly 3 featured posts
            const featuredPosts = resourcesData.getAllPosts().filter(post => post.featured).slice(0, 3);
            
            homeResourcesGrid.innerHTML = featuredPosts.map(post => {
                ...
                return `
                    <article class="resource-card" data-platform="${post.platform}" data-url="${post.url}">
                        <div class="resource-thumbnail">
                            <img src="${post.image}" alt="${post.title}" loading="lazy">
                            <div class="resource-platform" title="${platformName}">
                        ...
                        <div class="resource-content">
                            <h3 class="resource-title">${post.title}</h3>
                            <p class="resource-excerpt">${post.excerpt}</p>
                            <div class="resource-meta">
                                <span class="resource-date">${post.date}</span>
                                <span class="resource-reading-time">${post.readingTime}</span>
                            </div>
                            <a href="${post.url}" class="resource-link" target="_blank" rel="noopener noreferrer">
            ...
            // Setup click handlers for home resource cards
            const homeResourceCards = homeResourcesGrid.querySelectorAll('.resource-card');
            homeResourceCards.forEach(card => {
                card.addEventListener('click', (e) => {
                    // Don't trigger if clicking on the link directly
                    if (e.target.tagName === 'A' || e.target.closest('a')) {
                        return;
    ...
    setupHomeProjects() {
        ...
        if (homeProjectsGrid && typeof projectsData !== 'undefined') {
            // Get the latest 3 projects
            const latestProjects = getLatestProjects(3);
            
            homeProjectsGrid.innerHTML = latestProjects.map(project => {
                    ...
                    <article class="project-card">
                        <div class="project-icon">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                ${this.getProjectIcon(project.icon)}
                            </svg>
                        </div>
